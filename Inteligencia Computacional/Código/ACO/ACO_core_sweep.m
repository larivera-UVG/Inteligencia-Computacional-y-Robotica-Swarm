...
sweep = 1:1:50;
repetitions = 10;
core_sweep_data = cell(numel(sweep) * repetitions + 1, 5);
core_sweep_data(1, :) = {'tiempo', 'costo', 'iteraciones', 'path', 'cores'};
sweep_count = 1;
t_acumulado = 0;
...
if (t > t_max)
    ...
    core_sweep_data(sweep_count + 1, :) = {tiempofinal, 0, t - 1, best_path, cores};
else
    moda =  mode(L(:, t-1));
    ...
    tiempofinal = toc(timer);
    core_sweep_data(sweep_count + 1, :) = {tiempofinal, L(len_prob, t-1), t - 1, best_path, cores};
end
t_acumulado = t_acumulado + tiempofinal;
if t_acumulado >= 60
    disp("Guardando...\n")
    save('sweep_data', 'core_sweep_data', '-append')
    t_acumulado = 0;
end
...
% end_mail();
% sum(cell2mat(core_sweep_data(2:end, 1))) % Tiempo total de operación de la
% prueba (barrido)
