...
if iteraciones == 1
    ...
    ant_data = cell2table(ant_sweep_data(2:end, :), 'VariableNames', {'tiempo','costo','iteraciones','path','ant'});
    valores = {'50','60','70','80','90','100'};
    [grupo, id] = findgroups(ant_data.ant);
    func = @(p, ant, r) [mean(p), mean(ant), sum(r==2.5), sum(r==0)];
    result = splitapply(func, ant_data.tiempo, ant_data.iteraciones, ant_data.costo, grupo);
    agrupada = array2table([id, result], 'VariableNames', {'ant','tiempo','iteraciones', 'costo', 'fallos'});
